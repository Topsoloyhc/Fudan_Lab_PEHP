...
class Cropper(object):
    ...
    def crop(self):
        fileindexs = self.readImgName("train_id")
    ...
    def cropOneImg(self, fileindex):
        canvas, instance, imgHid = self.getInfo(fileindex)
        instanceNum = len(np.unique(imgHid))
        for instance_index in range(1, instanceNum):
            oricanvas, oriinstance, oriimgHid = self.getInfo(fileindex)
            ...
            newBbox, oldBbox, canvas, instance, imgHid = self.cropInstance(mask, oricanvas, oriinstance, oriimgHid)
            jasondata = CropJson.saveJson(instance_index, fileindex, fileindexnew, oldBbox, newBbox)
            self.saveImgs(imgHid, jasondata, canvas, instance, fileindexnew)
            self.__writeTxt(fileindexnew)

    def cropInstance(self, mask, canvas, instance, imgHid):
        ...
        if findresult[0].size > 0:
            ...
            xmin, ymin, neww, newh, oldBbox = self.calculateXY(canvas, xmin, ymin, xmax, ymax)
        ...
        canvas[mask != True] = [0, 255, 0]
        instance[mask != True] = [255, 255, 255]
        return newBbox, oldBbox, canvas, instance, imgHid

    def calculateXY(self, canvas, xmin, ymin, xmax, ymax):
        percentage = self.__config['percentage']
    ...
    def saveImgs(self, newHid, newjsondata, newimg, newinstance, fileindexnew):
        ...
        self.writeJsonFile(newjsondata, writepath + 'jsonfile/json' + fileindexnew + '.json')

    def writeJsonFile(self, dictwrite, outjsonfile):
