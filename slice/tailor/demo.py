...
def trans(keypoints):
    ...
    for i in range(len(keypoints)):
        tmp=keypoints[i]
        #print(tmp)
        tmp1=list(chain.from_iterable(tmp[0]))
        tmp1.append(tmp[1])
        tmp1=np.array(tmp1)
        tmp2=np.reshape(tmp1,[1,tmp1.size])
        if pose is None:
            pose=tmp2
        else:
            pose=np.vstack((pose,tmp2))
    return pose


if __name__ == '__main__':

    # allfilepath='/root/dataset/CIHP/slice/train_id.txt'
    # save_path='/root/model/simplepose/Improved-Body-Parts/testresult/'
    # image_path='/root/dataset/CIHP/slice/Images/'


    allfilepath = args.indexsfile
    image_path = args.inputdir
    save_path = args.outputdir
    allfile = readall(allfilepath)
    ...
    posenet.load_state_dict(checkpoint['weights'])  # 閸旂姴鍙嗘禒鏍︽眽鐠侇厾绮岄惃鍕侀崹瀣剁礉閸欘垵鍏橀棁鈧憰浣告嫹閻ｃ儵鍎撮崚鍡楃湴閿涘苯鍨痵trict=False
    ...
    for i in allfile:
        ...
        inputid = str(i)
        name = "%010d" % inputid
        print('processing ', name, '.jpg', count, '/', file_num)
        output = save_path + '/SPpose' + name + '.jpg'
        input_image = image_path + "/" + name + '.jpg'
        outposefile = save_path + '/SP' + name + '.txt'

        pose = None
        with torch.no_grad():
            canvas, keypoints = process(input_image, params, model_params, config.heat_layers + 2,
                                        config.paf_layers)  # todo background + 2
            # outposefile
            if keypoints is not None:
                pose = trans(keypoints)

        if pose is not None:
            np.savetxt(outposefile, pose, fmt='%f', delimiter=',')
